...
export async function sendEmail(options: SendEmailOptions): Promise<SendEmailResult> {
  const fromAddress = process.env.EMAIL_FROM || 'FluxIO Reports <reports@fluxio.com>'
...
export async function sendDailyReportEmail(
  ...
  const subject = `FluxIO Daily Report - ${companyName} - ${reportDate}`
    ...
    <html>
      ...
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #1f2937; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #1e3a5f 0%, #0e7490 100%); padding: 24px; border-radius: 12px 12px 0 0;">
          <h1 style="color: white; margin: 0; font-size: 24px;">FluxIO SCADA</h1>
        ...
        <div style="background: #1e293b; padding: 16px 24px; border-radius: 0 0 12px 12px; text-align: center;">
          <p style="color: rgba(255,255,255,0.6); margin: 0; font-size: 12px;">
            Generated by FluxIO SCADA | Version 3.3
          </p>
        </div>
      </body>
    </html>
  ...
  const text = `
FluxIO Daily Report - ${companyName} - ${reportDate}
...
This is an automated report. For questions or issues, please contact your system administrator.
  ...
  return sendEmail({
    ...
    attachments: [
      {
        filename: `FluxIO-Daily-Report-${reportDate}.pdf`,
