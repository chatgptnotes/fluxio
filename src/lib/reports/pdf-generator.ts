...
export function generatePDFReport(reportData: ReportData): string {
  ...
  doc.text('FluxIO SCADA', margin, 20)

  // Subtitle
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text('Flow Monitoring System Report', margin, 28)

  // Report type badge
  const reportTypeLabel = reportData.reportType.charAt(0).toUpperCase() + reportData.reportType.slice(1) + ' Report'
  doc.setFillColor(...COLORS.primary)
  doc.roundedRect(pageWidth - margin - 50, 12, 50, 10, 2, 2, 'F')
  doc.setFontSize(9)
  doc.text(reportTypeLabel, pageWidth - margin - 25, 18.5, { align: 'center' })
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFontSize(7)
    doc.text(
      `Generated by FluxIO SCADA | Version 1.3 | ${formatDateTime(new Date())}`,
      margin,
...
export function generatePDFBuffer(reportData: ReportData): Uint8Array {
  ...
  doc.text(`Generated: ${formatDateTime(reportData.generatedAt)}`, pageWidth - margin, 37, { align: 'right' })
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(...COLORS.textMuted)
    doc.setFontSize(7)
    doc.text(`Generated by FluxIO SCADA | Version 1.3 | ${formatDateTime(new Date())}`, margin, doc.internal.pageSize.getHeight() - 10)
